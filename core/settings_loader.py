...
def load_settings():
    ...
    # LLM fallback overrides
    if "fallback" not in settings["llm"]:
        settings["llm"]["fallback"] = {}
    if os.getenv("LLM_FALLBACK_PROVIDER"):
        settings["llm"]["fallback"]["provider"] = os.getenv("LLM_FALLBACK_PROVIDER")
    if os.getenv("LLM_FALLBACK_MODEL_NAME"):
        settings["llm"]["fallback"]["model_name"] = os.getenv("LLM_FALLBACK_MODEL_NAME")
    if os.getenv("LLM_FALLBACK_BASE_URL"):
        settings["llm"]["fallback"]["base_url"] = os.getenv("LLM_FALLBACK_BASE_URL")
    if os.getenv("OLLAMA_BASE_URL"):
        settings["llm"]["fallback"]["base_url"] = os.getenv("OLLAMA_BASE_URL")
    if os.getenv("LLM_FALLBACK_TIMEOUT"):
        settings["llm"]["fallback"]["timeout"] = int(os.getenv("LLM_FALLBACK_TIMEOUT"))
    ...
    if os.getenv("MIN_RELEVANCE_SCORE"):
        settings["reranking"]["min_relevance_score"] = float(os.getenv("MIN_RELEVANCE_SCORE"))

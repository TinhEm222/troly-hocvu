...
def upsert_chunks(chunks: list[dict]):
    ...
    # Xoa collection cu (neu co) truoc khi nap lai du lieu moi.
    # Ly do: pipeline nap toan bo du lieu tu dau moi lan chay (khong incremental),
    # va moi chunk duoc gan chunk_id ngau nhien moi -> neu khong xoa collection cu,
    # du lieu cu/trung lap se ton dong vinh vien trong Qdrant, gay nhieu cho retrieval
    # (vi du: du lieu cua lan ingest truoc do van con lan voi du lieu moi).
    try:
        client.delete_collection(collection_name=COLLECTION_NAME)
        logger.info(f"Deleted existing collection '{COLLECTION_NAME}' before re-ingesting fresh data.")
    except Exception as e:
        logger.info(f"No existing collection to delete (safe to ignore on first run): {e}")

    ensure_collection(client)

    if not chunks:
        logger.warning("No managed document chunks provided; collection was reset to empty.")
        return []
